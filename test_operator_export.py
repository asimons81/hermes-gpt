...
def test_export_returns_mcp_embedded_blob_without_local_path(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        assert wire(result)["structuredContent"] is not None
        metadata = wire(result)["structuredContent"]
        ...
        assert wire(embedded.resource)["mimeType"] == metadata["mime_type"]
...
def test_export_requires_workspace_level(monkeypatch, tmp_path) -> None:
    ...
    assert wire(result)["structuredContent"]["code"] == "FILE_EXPORT_ERROR"
...
def test_export_rejects_file_outside_allowed_root(monkeypatch, tmp_path) -> None:
    ...
    assert wire(result)["isError"] is True
    assert not any(isinstance(item, EmbeddedResource) for item in result.content)
    assert str(outside) not in json.dumps(wire(result)["structuredContent"])
...
def test_export_rejects_symlink_escape_outside_allowed_root(monkeypatch, tmp_path) -> None:
    ...
    assert wire(result)["isError"] is True
    assert not any(isinstance(item, EmbeddedResource) for item in result.content)
    assert "%PDF-outside" not in json.dumps(wire(result)["structuredContent"])
...
def test_export_rejects_secret_path_even_under_allowed_root(monkeypatch, tmp_path) -> None:
    ...
    assert wire(result)["isError"] is True
    assert not any(isinstance(item, EmbeddedResource) for item in result.content)
    assert "do-not-export" not in json.dumps(wire(result)["structuredContent"])
...
def test_export_optional_extension_allowlist(monkeypatch, tmp_path) -> None:
    ...
    assert wire(denied_result)["isError"] is True
    assert wire(allowed_result)["isError"] is False
