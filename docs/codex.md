...
Execution requires Operator Mode at `workspace` or acknowledged `owner` level, direct apply mode, an approved work directory, `HERMES_GPT_ENABLE_CODEX_RUNNER=1`, `confirm=true`, and `dry_run=false`. `execution_mode="normal"` is the default. `execution_mode="nolo"` is an explicit per-job opt-in that adds Codex 0.147.0's `-a never` approval policy while retaining the requested `read-only` or `workspace-write` sandbox. NOLO therefore prevents Codex from stopping for approval prompts without disabling sandbox containment. Hermes still validates the requested launch directory against `HERMES_GPT_OPERATOR_ALLOWED_PATHS` and still requires direct mode, `confirm=true`, and `dry_run=false`. `HERMES_GPT_ALLOW_CODEX_WRITE=1` is required whenever `workspace-write` is requested; read-only NORMAL and read-only NOLO jobs do not need the write gate. NOLO expires with the job and does not create persistent global approval-bypass state.
...
Jobs use fixed arguments, `shell=False`, bounded timeouts, prompt hashes instead of raw prompt persistence, and redacted bounded results. Arbitrary commands/config, caller-supplied executable paths, extra directories, and `danger-full-access` sandbox selection remain unsupported. The explicit job-scoped `execution_mode="nolo"` changes only Codex's approval policy to `never`; it does not disable the selected Codex sandbox and does not bypass Hermes Operator path or launch-policy checks.
